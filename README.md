...
这是一个用于 NodeSeek 论坛自动签到的工具，支持通过 Docker Compose 进行定时自动签到操作。签到模式默认为随机签到，帮助用户轻松获取论坛每日"鸡腿"奖励。
...
你需要根据注释提示，填写以下关键信息：
- **账户信息**: `NS_USER1`, `NS_PASS1`, `NS_USER2`, `NS_PASS2` 等。
...
| `NS_USER` 或 `USER` | 可选 | - | NodeSeek 论坛用户名（单账号） |
| `NS_PASS` 或 `PASS` | 可选 | - | NodeSeek 论坛密码（单账号） |
| `NS_USER1`、`NS_USER2`... | 可选 | - | NodeSeek 论坛用户名（多账号，也支持 `USER1`、`USER2`...） |
| `NS_PASS1`、`NS_PASS2`... | 可选 | - | NodeSeek 论坛密码（多账号，也支持 `PASS1`、`PASS2`...） |
...
| 变量名称 | 必要性 | 默认值 | 说明 |
| :------: | :----: | :----: | :--- |
| `RUN_AT` | 可选 | `08:00-10:59` | 设置定时任务执行时间，支持固定时间 `10:30` 或时间范围 `10:00-18:00` |
...
Deepflood 使用独立的环境变量前缀 `DF_`，配置结构与 NodeSeek 类似：

| 变量名称 | 说明 |
| :------: | :--- |
| `DF_COOKIE` | Deepflood Cookie，多账号使用`&`分隔 |
| `DF_USER` | Deepflood 论坛用户名（单账号） |
| `DF_PASS` | Deepflood 论坛密码（单账号） |
| `DF_USER1`、`DF_USER2`... | Deepflood 论坛用户名（多账号） |
| `DF_PASS1`、`DF_PASS2`... | Deepflood 论坛密码（多账号） |
| `DF_RANDOM` | 是否随机签到，默认 true |
| `DF_SOLVER_TYPE` | 验证码方案（不设置则继承全局 `SOLVER_TYPE`） |
| `DF_API_BASE_URL` | 验证码服务地址（不设置则继承全局 `API_BASE_URL`） |
| `DF_CLIENTT_KEY` | 验证码密钥（不设置则继承全局 `CLIENTT_KEY`） |
